<?php
declare(strict_types=1);

namespace EasyArray;

use EasyArray\Cloner\Cloner;
use EasyArray\TypeFactory\TypeFactory;
use EasyArray\TypeFactory\Type;

class EasyArray implements \ArrayAccess, \Iterator, \Countable
{
    /** @var int */
    public $position;

    /** @var array */
    public $items = [];

    /** @var bool */
    public $strict = false;

    /** @var TypeFactory */
    private $typeFactory;

    /** @var ?Type */
    private $type = null;

    /** @var Cloner */
    private $cloner;

    /**
     * When $strict is true, constructor will throw a TypeError 
     * if all items in $items are not the same type
     * 
     * Setting $strict to true, will make methods push(), set(), merge()
     * throw a TypeError, when argument of diffrent type the elements stored in $items
     * 
     * @param array $items
     * @param bool $strict
     */
    public function __construct(
        array $items,
        bool $strict = false
    ){
        $this->cloner = new Cloner();
        $this->typeFactory = new TypeFactory();

        $this->items = $items;
        $this->strict = $strict;
        $this->position = 0;

        if ($strict && count($items) >= 1) {
            foreach($items as $item) {
                if (! $this->hasTypeSet()) {
                    $this->type = $this->typeFactory->make($item);
                }

                $this->validateTypeOf($item);
            }
        }
    }

    public function __clone() 
    {
        $this->items = $this->cloner->clone($this->items);
    }

    /**
     * Returns an instance clone, also clones array internally stored items array 
     * calling clone() on each of its items
     * @return self
     */
    public function clone()
    {
        return clone($this);
    }

    public function isTyped(): bool
    {
        return $this->strict;
    }

    public function isNotTyped(): bool
    {
        return !$this->strict;
    }

    public function hasTypeSet(): bool
    {
        return !is_null($this->getType());
    }

    public function getType(): ?Type
    {
        return $this->type;
    }

    /**
     * Wrapper for php build in array_pop
     * @return mixed
     */
    public function pop()
    {
        return array_pop($this->items);
    }

    /**
     * Removes the last element of the array, thus shortening it
     * 
     * @return self
     */
    public function removeLast(): self
    {
        $this->pop();

        return $this;
    }

    /**
     * Add item at the end of the array
     * 
     * @param mixed $item
     * @return self
     */
    public function push($item): self
    {
        if ($this->isTyped()) {
            $this->updateTypeIfEmpty($item);
            $this->validateTypeOf($item);
        }

        $this->items[] = $item;

        return $this;
    }

    /**
     * Get the number of items in array
     * 
     * @return int
     */
    public function count(): int
    {
        return count($this->items);
    }

    /**
     * Set element stored under given index to a provided value
     * 
     * @return self
     */
    public function set($index, $value): self
    {
        if($this->isTyped()) {
            $this->updateTypeIfEmpty($value);
            $this->validateTypeOf($value);
        }

        $this->items[$index] = $value;
        
        return $this;
    }

    /**
     * Apply callback to each element of stored array
     * returns new instance of EasyArray
     * 
     * @param \Closure $closure
     * @return self
     */
    public function map(\Closure $closure): self
    {
        return $this->replicateSelfWith(
            array_map(
                $closure,
                $this->items
            )
        );
    }

    /**
     * Apply callback to each element of array
     * @param \Closure $closure
     * @return self
     */
    public function walk(\Closure $closure): self
    {
        foreach($this->items as $key => $item) {
            $this->items[$key] = call_user_func($closure, $item);
        }

        return $this;
    }

    /**
     * Returns the first item that satisfies the provided testing function.
     * If no values satisfy the testing function, null is returned. 
     * 
     * @param () => bool $callback
     * @return mixed|null
     */
    public function find(\Closure $callback)
    {
        foreach($this->items as $item) {
            if (boolval(call_user_func($callback, $item))) {
                return $item;
            }
        }

        return null;
    }

    /**
     * Returns index of the first item that satisfies the provided testing function.
     * If no values satisfy the testing function, null is returned. 
     * @return mixed|null
     */
    public function findIndex(\Closure $callback): ?int
    {
        foreach($this->items as $key => $item) {
            if (boolval(call_user_func($callback, $item))) {
                return $key;
            }
        }

        return null;
    }

     /**
      * Remove items form array that do no satisfy the provided filter function.
      * @param \Closure $callback
      * @return self
      */
    public function filter(\Closure $callback): self
    {
        $filteredItems = [];
        foreach($this->items as $item) {
            if(boolval(call_user_func($callback, $item))) {
                $filteredItems[] = $item;
            }
        }

        return $this->replicateSelfWith($filteredItems);
    }

    public function filterNull(): self
    {
        
    }

    public function flatten(int $levels = 5): self
    {
        $flat = [];

        foreach($this->items as $value) {
            if(is_array($value) && $levels >= 1) {
                $easy = new self($value);
                $flat = array_merge($flat, $easy->flatten($levels - 1)->values());

            } else {
                $flat[] = $value;
            }
        }

        return $this->replicateSelfWith($flat);
    }

    /**
     * Reverse the orider of stored items collection
     * 
     * @return self
     */
    public function reverse(): self
    {
        $this->items = array_reverse($this->items);

        return $this;
    }

    /**
     * Returns new instance with reversed order of items
     * @return self
     */
    public function reversed(): self
    {
        $newInstance = $this->replicateSelfWith($this->items);

        return $newInstance->reverse();
    }

    /**
     * If the number of rotations is positive is equivalent to successively calling $array->push($array->shift()) 
     * or if the number is negative $array->unshift($array->pop()) . 
     * 
     * @param int @rotations
     * @return self
     */
    public function rotate(int $rotations): self
    {
        if ($rotations > 0) {
            $this->push($this->shift());
        }

        if ($rotations < 0) {
            $this->unshift($this->pop());
        }

        return $this;
    }

    /**
     *  Will throw type if strict == true and if types do not math 
     * 
     * @throws \TypeError
     * @return self
     */
    public function diff(self $other): self
    {
        if($this->isTyped()
            && !$this->hasSameTypeAs($other)
        ) {
            throw new \TypeError("Types of provided array does not match the stored type.");
        }

        return $this->replicateSelfWith(
            array_diff($this->items, $other->items)
        );
    }

    /**
     * Returns the array of common elements of two arrays
     * Will throw type if strict == true and if types do not math 
     * 
     * @return self
     */
    public function intersect(self $other): self
    {
        if($this->isTyped()
            && !$this->hasSameTypeAs($other)
        ) {
            throw new \TypeError("Types of provided array does not match the stored type.");
        }

        return $this->replicateSelfWith(
            array_intersect($this->items, $other->items)
        );
    }

    /**
     * Alias for clone
     * 
     * @return self
     */
    public function copy(): self
    {
        return $this->clone();
    }

    /**
     * Removes item in the first position in array and returns it,
     * thus shortening the array by one element and moving everything down
     * 
     * @return mixed
     */
    public function shift()
    {
        return array_shift($this->items);
    }

    /**
     * Unsets the first element of the array, and moves everything down
     * 
     * @return self
     */
    public function removeFirst(): self
    {
        $this->shift();

        return $this;
    }

    public function unshift($item): self
    {
        if($this->isTyped()) {
            $this->updateTypeIfEmpty($item);
            $this->validateTypeOf($item);
        }

        array_unshift($this->items, $item);

        return $this;
    }

    /**
     * Each element of the array will passed as argument to provided callback, 
     * and stored under key that callback provides
     * 
     * callback that provides the key, must return something that can be used as a key
     * 
     * @param ?\Closure $keyProvider
     */
    public function keyBy(?\Closure $keyProvider): self
    {
        $newArray = [];

        foreach($this->items as $key => $item) {
            $newKey = call_user_func($keyProvider, $item);
            $newArray[$newKey] = $item;
        }

        $this->items = $newArray;

        return $this;
    }

    public function keySort(?\Closure $sortFunc = null): self
    {

    }

    /**
     * Sorts the stored array using the provided function, if not function is provided
     * if no sorting function is provided php build-in sort() is called
     * 
     * sorted array does not retain previous keys
     * 
     * The comparison function must return an integer less than, equal to, or greater than zero 
     * if the first argument is considered to be respectively less than, equal to, or greater than the second.
     * 
     * @param ?\Closure $sortFunc = null
     * @return self 
     */
    public function sort(?\Closure $sortFunc = null): self
    {
        if(is_null($sortFunc)) {
            sort($this->items);
        } else {
            usort($this->items, $sortFunc);
        }

        return $this;
    }

    /**
     * Creates new instance and sorts the array with provided callback
     * if no sorting function is provided php build-in sort() is called
     * 
     * sorted array does not retain previous keys
     * 
     * @param ?\Closure $sortFunc
     */
    public function sorted(?\Closure $sortFunc = null): self
    {
        $newInstance = $this->replicateSelfWith(
            $this->items
        );

        return $newInstance->sort($sortFunc);
    }

    /**
     * Returns a slice of an array, wrapped in EasyArray
     * if the offset is larger than the size of the array, an empty array is returned
     * 
     * Wrapper for php array_slice built-in function
     * 
     * @param int $offset
     * @param ?int $length
     * @param bool $pre
     */
    public function slice(
        int $offset,
        ?int $length = null,
        bool $preserveKeys = false
    ): self {
        return $this->replicateSelfWith(
            array_slice(
                $this->items, 
                $offset, 
                $length, 
                $preserveKeys
            )
        );
    }

    /**
     * Check if value is present in the array
     * wrapper for php built-in in_array()
     * @param mixed $value
     * @return bool
     */
    public function contains($value): bool
    {
        if ($this->isEmpty()) {
            return false;
        }

        if($this->isTyped()
            && !$this->hasSameTypeAsValue($value)
        ) {
            return false;
        }
    
        return in_array($value, $this->items);
    }

    /**
     * Checks if each element of other array is present in this array
     * value order is ignored
     * 
     * @return bool
     */
    public function includes(self $other): bool
    {
        if( $this->isTyped()
            && !$this->hasSameTypeAs($other)
        ) {
            return false;
        }

        if ($other->count() > $this->count()) {
            return false;
        }

        foreach($other->items() as $item) {
            if (! $this->contains($item)) {
                return false;
            }
        }

        return true;
    }

    /**
     * Checks if each element of other array is present in this array 
     * and both arrays have elements in common part in the same order
     * 
     * @return bool
     */
    public function includesInOrder(self $other): bool
    {
        if( $this->isTyped()
            && !$this->hasSameTypeAs($other)
        ) {
            return false;
        }

        if ($other->count() > $this->count()) {
            return false;
        }

        foreach($this->items() as $item) {
            if($item === $other->first()) {
                for($i = 0; $i < $other->count();  $i++) {
                    if ($this->values()[$i] != $other->values()[$i]) {
                        return false;
                    }
                }

                return true;

            } else {
                return $this
                    ->copy()
                    ->removeFirst()
                    ->includesInOrder($other)
                ;
            }
        }

        return false;
    }

    /**
     * Checks if both arrays have the same length
     * and store elements thata are equal in the same order
     * 
     * @return bool
     */
    public function isSameAs(self $other): bool
    {
        return $this->count() == $other->count()
            && $this->includesInOrder($other)
        ;
    }

    public function hasSameTypeAs(self $other): bool
    {
        if($this->hasTypeSet()
            && $other->hasTypeSet()
        ) {
            return $this->type->equals($other->getType());
        }

        return false;
    }

    public function hasSameTypeAsValue($value): bool
    {
        if($this->isNotTyped()) {
            return false;
        }

        if (! $this->hasTypeSet()) {
            throw new \Exception("No type set");
        }

        $valueType = $this->typeFactory->make($value);
        return $this->type->equals($valueType);
    }

    public function setType(Type $type): self
    {
        $this->type = $type;

        return self;
    }

    public function setTypeFrom($value): self
    {
        if($this->isNotTyped()) {
            throw new \Exception("Can not set type, not a typed array.");
        }

        if ($this->isTyped()
            && $this->hasTypeSet()
        ) {
            throw new \Exception("Type already set to: " . $this->type->getTypeName());
        }

        $this->type = $this->typeFactory->make($value);

        return $this;
    }

    /**
     * Fetch first element of array
     * Returns null if $items is empty
     * 
     * @return mixed|null
     */
    public function first()
    {
        if(count($this->items) == 0) {
            return null;
        }

        return array_values($this->items)[0];
    }

    /**
     * Fetch last element of array
     * returns null if $items is empty
     * 
     * @return mixed|null
     */
    public function last()
    {
        if(count($this->items) == 0) {
            return null;
        }

        $index = count($this->items);

        return array_values($this->items)[$index - 1];
    }

    /**
     * Will merge two instances of EasyArray, returns new instance of EasyArray
     * 
     * When type checking is enabled will throw TypeError in type of values in other array
     * do not match the stored type
     * 
     * @param self $other
     * @return self new instance of EasyArray
     * @throws \TypeError
     */
    public function merge(self $other): self
    {
        if ($this->isTyped()
            && !$this->hasTypeSet()
            && $other->isNotEmpty()
        ) {
            $this->setTypeFrom($other->first());
        }
        
        if ($this->isTyped()) {
            foreach($other as $otherItem) {
                $this->validateTypeOf($otherItem);
            }
        }

        return $this->replicateSelfWith(
            array_merge($this->items, $other->items())
        );
    }

    /**
     * Wrapper for php build in array_values()
     * 
     * @return array
     */
    public function values(): array
    {
        return array_values($this->items);
    }

    /**
     * Wrapper for build in array_keys()
     * 
     * @return array
     */
    public function keys(): array
    {
        return array_keys($this->items);
    }

    /**
     * Returns internally stored array
     * @return array
     */
    public function items(): array
    {
        return $this->items;
    }

    /**
     * Wrapper for php build in array_key_exists_function
     * @param mixed $key
     */
    public function arrayKeyExists($key): bool
    {
        return array_key_exists($key, $this->items);
    }

    /**
     * Retrives a value stored under given index, 
     * if not found null is returned
     * @param int|string $index
     * @return mixed|null
     */
    public function get($index)
    {
        if (isset($this->items[$index])) {
            return $this->items[$index];
        }
        return null;
    }

    /**
     * Return index of 
     * @param int | string $index
     * @return mixed|null
     */
    public function getIndex($searched)
    {
        if ($this->hasTypeSet()
            && !$this->hasSameTypeAsValue($searched)
        ) {
            return null;
        }

        foreach($this->items as $index => $item) {
            if($searched === $item) {
                return $index;
            }
        }

        return null;
    }

    public function current()
    {
        return $this->items[$this->position];
    }

    public function next(): void
    {
        ++$this->position;
    }

    public function key()
    {
        return $this->position;
    }

    public function valid()
    {
        return isset($this->items[$this->position]);
    }

    public function rewind()
    {
        $this->position = 0;
    }

    public function offsetExists($offset): bool
    {
        return isset($this->items[$offset]);
    }

    public function offsetGet($offset)
    {
        return isset($this->items[$offset]) ? $this->items[$offset] : null;
    }

    public function offsetSet($offset, $value): void
    {
        if($this->isTyped()) {
            $this->updateTypeIfEmpty($value);
            $this->validateTypeOf($value);
        }

        if (is_null($offset)) {
            $this->items[] = $value;
        } else {
            $this->items[$offset] = $value;
        }
    }

    public function offsetUnset($offset): void
    {
        unset($this->items[$offset]);
    }

    public function isNotEmpty(): bool
    {
        return $this->count() >= 1;
    }

    public function isEmpty(): bool
    {
        return $this->count() == 0;
    }

    private function updateTypeIfEmpty($value): self
    {
        if($this->isEmpty()
            && $this->isTyped()
        ) {
            $this->setTypeFrom($value);
        }

        return $this;
    }

    private function validateTypeOf($value): void
    {
        if ($this->isNotTyped()) {
            return;
        }

        if ($this->hasTypeSet()
            && !$this->hasSameTypeAsValue($value)
        ) {
            throw new \TypeError(
                "Wrong type of value, expecting: " . $this->type->getTypeName()
                . " recived: " . $this->typeFactory->make($value)->getTypeName()
            );
        }
    }

    private function replicateSelfWith(array $items = []): self
    {
        return new self($items, $this->isTyped());
    }
}
